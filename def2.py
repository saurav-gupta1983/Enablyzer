...
db = pymysql.connect("localhost","root","0809","Enablyzer")
...
sql_qry = """CREATE TABLE codeCalledfunc(CodeCalledFuncId bigint(20) not null auto_increment,
...
for f in files:
        ...
        for i,l in enumerate(lines_f):
            ...
            called_func=re.findall('[(A-Z)\(a-z)\\_][(A-Z)\(a-z)\\_\(0-9)]*\([^\)]*\)',l)
            #print(called_func)
            ln=len(called_func)
            while ln:
                ...
                for j in range(0,len(cf)-1):
                    #print(cf[j])
                    cf1=re.findall('[(A-Z)\(a-z)\\_][(A-Z)\(a-z)\\_\(0-9)]*',cf[j])
                    if cf1:
                        cf[j]=cf1[0].strip()
                        
                        #print(cf[j])
                        if cf[j]=='if' or cf[j]=='while' or cf[j]=='for' or not re.search('[a-zA-Z]', cf[j]):
                            continue
                        cflist.append(cf[j])
        ...
        try:
            for beg,end,rem,lst in scope:
                x=beg-1
                #print (lines_f[x],x)
                func=re.findall(r'[(A-Z)\(a-z)\_][(A-Z)\(a-z)\_\(0-9)]*\([^\)]*\)',lines_f[x])
                if func:
                    #print(func[0])
